...
export default function NoteEditorPage() {
  ...
  // Export to PDF — generates a styled printable document and triggers browser print/save
  const handleExportPDF = useCallback(() => {
    if (sections.length === 0) return;

    const patientName =
      (pageState.note?.generation_metadata as Record<string, unknown>)?.patient_name || "Patient";
    const visitType =
      (pageState.note?.generation_metadata as Record<string, unknown>)?.visit_type || "";
    const template =
      (pageState.note?.generation_metadata as Record<string, unknown>)?.template || "Clinical Note";
    ...
    const billingHtml =
      acceptedCodes.length > 0
        ? `<div class="section"><h2>Billing Codes</h2><table><tr><th>Code</th><th>System</th><th>Description</th></tr>${acceptedCodes
            .map(
              (c) =>
                `<tr><td><strong>${c.code}</strong></td><td>${c.system}</td><td>${c.description}</td></tr>`
            )
            .join("")}</table></div>`
        : "";
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  ${sectionsHtml}
  ${billingHtml}
  <div class="footer">Generated by MindCare AI &mdash; ${createdAt}</div>
</body></html>`;
  ...
  }, [sections, billingCodes, pageState.note, status]);
