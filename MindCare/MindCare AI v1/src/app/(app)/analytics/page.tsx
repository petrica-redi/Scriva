...
export default function AnalyticsPage() {
  ...
  const generateReport = () => {
    const lines: string[] = [];
    lines.push("ANALYTICS REPORT — MindCare AI");
    ...
    <div className="flex flex-col gap-6 p-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              <div>
                <label className="block text-xs font-medium text-medical-muted mb-1">Gender</label>
              ...
              <div>
                <label className="block text-xs font-medium text-medical-muted mb-1">Visit Type</label>
              ...
              <div>
                <label className="block text-xs font-medium text-medical-muted mb-1">ICD Code / Diagnosis</label>
              ...
              <div>
                <label className="block text-xs font-medium text-medical-muted mb-1">Patient ID</label>
              ...
              <div>
                <label className="block text-xs font-medium text-medical-muted mb-1">From Date</label>
              ...
              <div>
                <label className="block text-xs font-medium text-medical-muted mb-1">To Date</label>
      ...
      <div className="flex gap-1 border-b border-medical-border">
        {([
          ...
          { key: "reports" as const, label: "Reports & ICD Analysis" },
        ]).map((tab) => (
          <button
            key={tab.key}
      ...
      {loading ? (
        <div className="py-12 text-center text-medical-muted">Loading analytics...</div>
      ) : activeTab === "overview" ? (
        <>
          {/* Summary Stats */}
          <div className="grid gap-4 md:grid-cols-4">
            <Card>
              <CardContent className="pt-6">
                <p className="text-sm text-medical-muted">Total Consultations</p>
                <p className="mt-2 text-3xl font-bold text-medical-text">{totalCount}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <p className="text-sm text-medical-muted">This Month</p>
                <p className="mt-2 text-3xl font-bold text-blue-600">{monthCount}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <p className="text-sm text-medical-muted">This Week</p>
                <p className="mt-2 text-3xl font-bold text-indigo-600">{weekCount}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-6">
                <p className="text-sm text-medical-muted">Avg. Duration</p>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card>
              <CardHeader><CardTitle>Consultations This Week</CardTitle></CardHeader>
              <CardContent>
            ...
            {/* Note Status */}
            <Card>
              <CardHeader><CardTitle>Note Status Distribution</CardTitle></CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {[
                    { label: "Draft", count: draftCount, color: "bg-yellow-400" },
                    { label: "Reviewed", count: reviewedCount, color: "bg-blue-400" },
                    { label: "Finalized", count: finalizedCount, color: "bg-green-400" },
                  ].map((item) => {
            ...
            {/* Visit Types */}
            <Card>
              <CardHeader><CardTitle>Visit Types</CardTitle></CardHeader>
              <CardContent>
                {visitTypes.length > 0 ? (
                  ...
                  <p className="text-sm text-medical-muted text-center py-4">No consultations yet</p>
                )}
              </CardContent>
            </Card>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>Top ICD-10 Diagnosis Codes</CardTitle>
                <span className="text-xs text-medical-muted">{Object.keys(icdCounts).length} unique codes</span>
              </div>
            </CardHeader>
            <CardContent>
              {topICD.length > 0 ? (
                ...
                <p className="text-sm text-medical-muted text-center py-6">No ICD codes found in clinical notes. Generate notes from consultations to populate this data.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader><CardTitle>Recent Clinical Notes</CardTitle></CardHeader>
            <CardContent>
              {notes.length > 0 ? (
                <div className="space-y-3">
                ...
                <p className="text-sm text-medical-muted text-center py-6">No clinical notes found.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-medical-text">Generate Custom Report</h3>
                  <p className="text-sm text-medical-muted mt-1">
                    Export a comprehensive analytics report based on your current filters.
                    {(filterGender || filterVisitType || filterICD || filterPatientId || filterDateFrom || filterDateTo) ?
                      " Active filters will be applied." : " Use the Filters button above to narrow results."}
                  </p>
                </div>
