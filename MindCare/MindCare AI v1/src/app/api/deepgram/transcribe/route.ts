...
export async function POST(request: NextRequest) {
  try {
    // Auth check (demo mode — always passes)
    // In production, restore real auth check here
    ...
    console.log(
      `[Deepgram] Transcribing audio: ${(audioBlob.size / 1024).toFixed(1)}KB, type: ${audioBlob.type}, mode: ${isMultichannel ? "multichannel" : "diarize"}, language: ${language}, model: ${primaryModel}`
    );
    ...
    if (!deepgramResponse.ok) {
      ...
      if (
        ...
        if (!fallbackResponse.ok) {
          ...
          return NextResponse.json(
            { error: `Deepgram transcription failed: ${fallbackErr}` },
            { status: 500 }
          );
        }
